...
Droplet actions are tasks that can be executed on a Droplet, such as rebooting, resizing, or snapshotting a Droplet.`,
	...
	AddIntFlag(cmdDropletActionGet, blcli.ArgActionID, "", 0, "Action ID", requiredOpt())
	...
	AddBoolFlag(cmdDropletActionEnableBackups, blcli.ArgCommandWait, "", false, "Wait for action to complete")
	...
	cmdDropletActionDisableBackups := CmdBuilder(cmd, RunDropletActionDisableBackups,
		...
		displayerType(&displayers.Action{}))
	AddBoolFlag(cmdDropletActionDisableBackups, blcli.ArgCommandWait, "", false, "Wait for action to complete")
	...
	cmdDropletActionReboot := CmdBuilder(cmd, RunDropletActionReboot,
		...
		displayerType(&displayers.Action{}))
	AddBoolFlag(cmdDropletActionReboot, blcli.ArgCommandWait, "", false, "Wait for action to complete")
	...
	cmdDropletActionPowerCycle := CmdBuilder(cmd, RunDropletActionPowerCycle,
		...
		displayerType(&displayers.Action{}))
	AddBoolFlag(cmdDropletActionPowerCycle, blcli.ArgCommandWait, "", false, "Wait for action to complete")
	...
	cmdDropletActionShutdown := CmdBuilder(cmd, RunDropletActionShutdown,
		...
		displayerType(&displayers.Action{}))
	AddBoolFlag(cmdDropletActionShutdown, blcli.ArgCommandWait, "", false, "Wait for action to complete")
	...
	cmdDropletActionPowerOff := CmdBuilder(cmd, RunDropletActionPowerOff,
		...
		displayerType(&displayers.Action{}))
	AddBoolFlag(cmdDropletActionPowerOff, blcli.ArgCommandWait, "", false, "Wait for action to complete")
	...
	cmdDropletActionPowerOn := CmdBuilder(cmd, RunDropletActionPowerOn,
		...
		displayerType(&displayers.Action{}))
	AddBoolFlag(cmdDropletActionPowerOn, blcli.ArgCommandWait, "", false, "Wait for action to complete")
	...
	cmdDropletActionPasswordReset := CmdBuilder(cmd, RunDropletActionPasswordReset,
		...
		displayerType(&displayers.Action{}))
	AddBoolFlag(cmdDropletActionPasswordReset, blcli.ArgCommandWait, "", false, "Wait for action to complete")
	...
	cmdDropletActionEnableIPv6 := CmdBuilder(cmd, RunDropletActionEnableIPv6,
		...
		displayerType(&displayers.Action{}))
	AddBoolFlag(cmdDropletActionEnableIPv6, blcli.ArgCommandWait, "", false, "Wait for action to complete")
	...
	cmdDropletActionEnablePrivateNetworking := CmdBuilder(cmd, RunDropletActionEnablePrivateNetworking,
		...
		displayerType(&displayers.Action{}))
	AddBoolFlag(cmdDropletActionEnablePrivateNetworking, blcli.ArgCommandWait, "", false, "Wait for action to complete")
	...
	cmdDropletActionRestore := CmdBuilder(cmd, RunDropletActionRestore,
		...
		displayerType(&displayers.Action{}))
	AddIntFlag(cmdDropletActionRestore, blcli.ArgImageID, "", 0, "Image ID", requiredOpt())
	AddBoolFlag(cmdDropletActionRestore, blcli.ArgCommandWait, "", false, "Wait for action to complete")
...
In order to resize a Droplet, it must first be powered off.`
	cmdDropletActionResize := CmdBuilder(cmd, RunDropletActionResize,
		...
		displayerType(&displayers.Action{}))
	AddBoolFlag(cmdDropletActionResize, blcli.ArgResizeDisk, "", false, "Resize the Droplet's disk size in addition to its RAM and CPU.")
	AddStringFlag(cmdDropletActionResize, blcli.ArgSizeSlug, "", "", "A slug indicating the new size for the Droplet (e.g. `s-2vcpu-2gb`). Run `doctl compute size list` for a list of valid sizes.", requiredOpt())
	AddBoolFlag(cmdDropletActionResize, blcli.ArgCommandWait, "", false, "Wait for action to complete")
	...
	cmdDropletActionRebuild := CmdBuilder(cmd, RunDropletActionRebuild,
		...
		displayerType(&displayers.Action{}))
	AddStringFlag(cmdDropletActionRebuild, blcli.ArgImage, "", "", "Image ID or Slug", requiredOpt())
	AddBoolFlag(cmdDropletActionRebuild, blcli.ArgCommandWait, "", false, "Wait for action to complete")
	...
	cmdDropletActionRename := CmdBuilder(cmd, RunDropletActionRename,
		...
		displayerType(&displayers.Action{}))
	AddStringFlag(cmdDropletActionRename, blcli.ArgDropletName, "", "", "Droplet name", requiredOpt())
	AddBoolFlag(cmdDropletActionRename, blcli.ArgCommandWait, "", false, "Wait for action to complete")
	...
	cmdDropletActionChangeKernel := CmdBuilder(cmd, RunDropletActionChangeKernel,
		...
		displayerType(&displayers.Action{}))
	AddIntFlag(cmdDropletActionChangeKernel, blcli.ArgKernelID, "", 0, "Kernel ID", requiredOpt())
	AddBoolFlag(cmdDropletActionChangeKernel, blcli.ArgCommandWait, "", false, "Wait for action to complete")
	...
	cmdDropletActionSnapshot := CmdBuilder(cmd, RunDropletActionSnapshot,
		...
		displayerType(&displayers.Action{}))
	AddStringFlag(cmdDropletActionSnapshot, blcli.ArgSnapshotName, "", "", "Snapshot name", requiredOpt())
	AddBoolFlag(cmdDropletActionSnapshot, blcli.ArgCommandWait, "", false, "Wait for action to complete")
...
func RunDropletActionRename(c *CmdConfig) error {
	fn := func(das do.DropletActionsService) (*do.Action, error) {
		...
		name, err := c.Doit.GetString(c.NS, blcli.ArgDropletName)
		if err != nil {
			return nil, err
		}
