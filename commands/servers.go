...
func Server() *Command {
	...
	bl compute server create --image ubuntu-20-04-x64 --size s-1vcpu-1gb --region nyc1 example.com
`
	...
	AddStringFlag(cmdServerCreate, blcli.ArgRegionSlug, "", "", "A slug indicating the region where the Server will be created (e.g. `nyc1`). Run `bl compute region list` for a list of valid regions.",
		requiredOpt())
	AddStringFlag(cmdServerCreate, blcli.ArgSizeSlug, "", "", "A slug indicating the size of the Server (e.g. `s-1vcpu-1gb`). Run `bl compute size list` for a list of valid sizes.",
		requiredOpt())
	...
	AddStringFlag(cmdServerCreate, blcli.ArgImage, "", "", "An ID or slug indicating the image the Server will be based-on (e.g. `ubuntu-20-04-x64`). Use the commands under `bl compute image` to find additional images.",
		requiredOpt())
